...
pa<-ggplot(i, aes(fct_reorder(Departamento,w_mean), w_mean, fill=Departamento))+
  geom_col()+ 
  coord_flip()+
  labs(y="Artículos", subtitle="Total en el periodo 2017,2018,2019 por investigador y  Promedio por año  ()", x="Departamento",title="Artículos tipo A por departamento académico.")+
  theme_light()+
  annotate("text", x="DEUMA", y=5, label="(.92), 2.76")+
  annotate("text", x="DEP", y=5, label="(1.08), 3.24 ")+
  annotate("text", x="DEC", y=5, label="(.40), 1.20")+
  annotate("text", x="DEAP", y=5, label="(0.15), 0.45")+
  annotate("text", x="DES", y=5, label="(.40),1.20")+
  annotate("text", x="DEE", y=5, label="(0.52), 1.56")+
  theme(legend.position='none')
...
**¿Es equilibrada la praticipación de los investigadores en este tipo de productos?**
