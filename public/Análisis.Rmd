...
l2<-ggplot(lib.media, aes(fct_reorder(Departamento,libros), libros, fill=Departamento))+
  geom_col()+
  coord_flip()+
  labs(y="Libros", subtitle="Por departamento académico, total SIEVA, 2017,2018,2019", x="Departamento",title="Libros de Autor")+
  theme_light()+
...
Es un tipo de investigación que es mas común en **DEC** y **DEAP**.

La inversión de tiempo necesaria para generar un libro es mayor que por ejemplo un artículo tipo A.

Hoy en día estos dos tipos de productos se valoran igual en el sistema de evaluación.

Importante considerar revalorar este rubro para incentivar su producción.

La producción de libros es ínfima, esto se percibe como **área de oportunidad**
...
Para abordareste punto se analiza a continuación la participación de los investigadores en la generación de publicaciones tipo A, la calificación de calidad más alta  establecida en la regulación vigente  (en conjunto con la elaboración de libros) y se hace el contraste por departamento.
...
pa<-ggplot(i, aes(fct_reorder(Departamento,w_mean), w_mean, fill=Departamento))+
  geom_col()+ 
  coord_flip()+
  labs(y="Artículos", subtitle="Promedio en el periodo: 2017,2018,2019", x="Departamento",title="Artículos tipo A por departamento académico.")+
  theme_light()+
